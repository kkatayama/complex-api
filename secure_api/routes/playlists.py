...
def create_playlist(*, db: Session = Depends(get_session), playlist: CreatePlaylist, me: User = Depends(get_currentUser)):
...
@playlists_router.get("/playlists/{playlistID}", response_model=PlaylistWithUser, tags=["Playlist"])
...
@playlists_router.get("/playlists/{playlistID}/tracks", response_model=PlaylistWithUser, tags=["Playlist"])
def read_playlist_tracks(*, db: Session = Depends(get_session), playlistID: int):
    playlist = db.get(Playlist, playlistID)
    if not playlist:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Playlist not found")
    return playlist
